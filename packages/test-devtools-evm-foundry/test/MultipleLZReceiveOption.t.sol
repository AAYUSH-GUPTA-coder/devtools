...
contract MultipleLZReceiveOptionTest is TestHelperOz5 {
    ...
    function test_assemblyDecode() public pure {
        uint128 gas = 2;
        uint128 value = 1;

        bytes memory payload = abi.encodePacked(gas, value);
        ...
        assembly {
            lz_gas := mload(add(payload, u128_LENGTH))
            lz_value := mload(add(payload, mul(u128_LENGTH, 2)))
        }

        assertEq(lz_gas, gas, "decoding of gas failed");
        assertEq(lz_value, value, "decoding of value failed");
    }

    function test_SingleParseExecutorLzReceiveOption_OnlyGas() public view {
    ...
    function test_SingleParseExecutorLzReceiveOption_GasValue() public view {
    ...
    function test_MultiParseExecutorLzReceiveOption_OnlyGas() public view {
    ...
    function test_MultiParseExecutorLzReceiveOption_GasValue() public view {
        bytes memory options = OptionsBuilder
            .newOptions()
